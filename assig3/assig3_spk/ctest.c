...
#define EMPLOYEE_STRUCT_STRING_SIZE 100
//define a struct
struct Employee {
  char name[EMPLOYEE_STRUCT_STRING_SIZE];
  int birth_year;
  int start_year;
};
...
struct Employee* make_employee(char* name, int birth_year, int start_year) {
  ...
  mystrcpy(newEmployee->name, name);
  newEmployee->birth_year = birth_year;
  newEmployee->start_year = start_year;
  return newEmployee;
}

/**Prints an Employee struct
@param anEmployee Pointer to an Emplyee Structure
@author Steven Kordell
*/
void print_employee(struct Employee* anEmployee) {
  printf("Employee:\tname = %-29s birth_year = %d\t start_year = %d  pointer = %p\n",anEmployee->name,anEmployee->birth_year,anEmployee->start_year,anEmployee);
}
...
*/
struct Employee* random_employee() {
  int birthYear = 1;            //temporary variable
  int startYear = 0;            //temproary variable
  while (birthYear > startYear) { //ensure starting year is after birth year
    birthYear = genRandInt(200) + 1800;
    startYear = genRandInt(200) + 1800;
  }
  return make_employee(genRandString(genRandInt(26)+3), birthYear, startYear); //make and return the stuct
}

/** makes an array of pointers to randomized employee structs
 ...
 */
struct Employee** deepCopyEmployees(struct Employee** src, int n) {
  int i = 0;  //define an iterator variable
  struct Employee** duplicate = (struct Employee**) malloc(sizeof(struct Employee*) * n); //allocate memory for the array of pointers
  while (i++ < n) { //iterate over array of pointers
    char *nameString = (char*) malloc(sizeof(char*)*EMPLOYEE_STRUCT_STRING_SIZE);     //make space in memory for the name field
    mystrcpy(nameString,(*src)->name);        //copy the name field
    *(duplicate++) = make_employee(nameString, (*src)->birth_year, (*src)->start_year);   //duplicate each structure and save the pointer
    src++;
  }
  src-=n;     //reset the src pointer to the array's start
  return duplicate-n;  //return the pointer to the duplicate array
}
